...
function Lightbox({
  ...
  const prev = useCallback(() => { setIdx((i) => (i - 1 + images.length) % images.length); resetZoom(); }, [images.length]);
  const next = useCallback(() => { setIdx((i) => (i + 1) % images.length); resetZoom(); }, [images.length]);
  ...
  const onMouseUp = useCallback(() => { dragStart.current = null; setDragging(false); }, []);
  ...
  return (
    <motion.div
      ...
      onWheel={onWheel}
    >
      <button className="absolute top-4 right-4 z-20 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors" onClick={onClose}>
        <X size={18} />
      </button>
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1.5">
        <button className="w-7 h-7 flex items-center justify-center text-white/70 hover:text-white transition-colors disabled:opacity-30"
        ...
        {zoom > 1 && (
          <button className="w-7 h-7 flex items-center justify-center text-white/70 hover:text-white transition-colors ml-1"
      ...
      >
        <AnimatePresence mode="wait">
          <motion.img
            ...
            style={{ transform: `scale(${zoom}) translate(${offset.x / zoom}px, ${offset.y / zoom}px)`, transition: dragging ? 'none' : 'transform 0.15s ease' }}
            draggable={false}
          />
        </AnimatePresence>
      </div>
      ...
      {images.length > 1 && zoom === 1 && (
        <>
          <button className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-11 h-11 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors"
            onClick={(e) => { e.stopPropagation(); prev(); }}>
            <ChevronLeft size={20} />
          </button>
          <button className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-11 h-11 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors"
      ...
      <div className="min-h-screen bg-bg-primary text-text-primary">
        ...
        <div className="relative w-full overflow-hidden" style={{ minHeight: '340px' }}>
          ...
          {project.cover_image ? (
            <div
              ...
              style={{ backgroundImage: `url(${project.cover_image})`, filter: 'blur(6px)' }}
            />
          ) : (
          ...
          <div className="relative z-10 w-full px-[10%] pt-28 pb-14">
            ...
            {/* Title */}
            <h1 className="font-syne font-bold text-4xl md:text-5xl text-text-primary mb-6 leading-tight max-w-3xl">
              {displayTitle}
            </h1>
        ...
        {allImages.length > 0 && (
          <div className="w-full px-[10%] pb-12">
            <h2 className="font-grotesk font-medium text-text-secondary text-xs tracking-widest uppercase mb-6">Gallery</h2>
            ...
            >
              ...
              {allImages.length > 1 && (
                <>
                  <button className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-sm border border-white/10 text-white hover:border-gold/50 hover:text-gold transition-all duration-200"
                    onClick={(e) => { e.stopPropagation(); setActiveIdx((i) => (i - 1 + allImages.length) % allImages.length); }}>
                    <ChevronLeft size={16} />
                  </button>
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-sm border border-white/10 text-white hover:border-gold/50 hover:text-gold transition-all duration-200"
                ...
                {allImages.map((img, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-12 rounded-md overflow-hidden border-2 transition-all duration-200 ${
                      i === activeIdx ? 'border-gold opacity-100' : 'border-transparent opacity-50 hover:opacity-80'
                    }`}
                  >
        ...
        <div className="w-full px-[10%] pb-20">
          <button
            onClick={handleBack}
            className="inline-flex items-center gap-2 text-text-secondary hover:text-gold border border-border hover:border-gold/50 rounded-lg px-5 py-2.5 font-grotesk text-sm transition-all duration-300"
          >
            <ArrowLeft size={14} />
